    ...
    <div className="max-w-[1200px] mx-auto px-sm py-lg grid grid-cols-1 lg:grid-cols-3 gap-md items-start">
          ...
          {activeStep === 1 && (
            ...
            >
              <div className="flex items-center gap-2xs border-b border-white/5 pb-xs">
                ...
                <h3 className="text-sm font-bold uppercase tracking-wider text-white">Passenger Document Verification</h3>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-sm">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder=" "
                  />
                  <label className="absolute text-xs text-vantage-muted left-xs top-xs scale-100 origin-top-left transition-all peer-placeholder-shown:scale-100 peer-placeholder-shown:top-sm peer-focus:scale-75 peer-focus:top-2xs peer-focus:text-vantage-accent">First Name</label>
                  {errors.firstName && <span className="text-[10px] text-red-400 block mt-3xs">{errors.firstName}</span>}
                </div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder=" "
                  />
                  <label className="absolute text-xs text-vantage-muted left-xs top-xs scale-100 origin-top-left transition-all peer-placeholder-shown:scale-100 peer-placeholder-shown:top-sm peer-focus:scale-75 peer-focus:top-2xs peer-focus:text-vantage-accent">Last Name</label>
                  {errors.lastName && <span className="text-[10px] text-red-400 block mt-3xs">{errors.lastName}</span>}
                </div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder=" "
                  />
                  <label className="absolute text-xs text-vantage-muted left-xs top-xs scale-100 origin-top-left transition-all peer-placeholder-shown:scale-100 peer-placeholder-shown:top-sm peer-focus:scale-75 peer-focus:top-2xs peer-focus:text-vantage-accent">Passport Number</label>
                  {errors.passportNumber && <span className="text-[10px] text-red-400 block mt-3xs">{errors.passportNumber}</span>}
                </div>
                ...
                <div className="relative">
                  ...
                  />
                  <label className="absolute text-[10px] text-vantage-accent left-xs top-2xs scale-75 origin-top-left">Date of Birth</label>
                  {errors.dateOfBirth && <span className="text-[10px] text-red-400 block mt-3xs">{errors.dateOfBirth}</span>}
                </div>
              </div>

              <div className="pt-xs flex justify-end">
                <button type="submit" className="px-md py-xs rounded-xl bg-vantage-accent text-vantage-dark font-bold text-xs tracking-wide uppercase hover:bg-white transition-colors duration-300">
                  Save & Continue
                </button>
              </div>
            </motion.form>
            ...
            >
              ...
              <div className="md:col-span-2 premium-glass p-sm rounded-2xl border border-white/5 bg-gradient-to-br from-vantage-surface to-vantage-slate/60 text-white min-h-[160px] flex flex-col justify-between shadow-xl relative overflow-hidden group">
                <div className="flex justify-between items-start">
                  <div className="w-8 h-6 bg-amber-400/20 rounded-md border border-amber-400/30 flex items-center justify-center font-mono text-[8px] tracking-widest text-amber-300 font-bold">CHIP</div>
                  <div className="text-xs font-bold uppercase italic tracking-widest text-white/40">
                    ...
                    {cardBrand === 'mastercard' && <span className="text-orange-400 font-black">MC</span>}
                    {cardBrand === 'amex' && <span className="text-emerald-400 font-black">AMEX</span>}
                ...
                <div className="flex justify-between items-center text-[10px] font-mono tracking-wider text-vantage-muted uppercase">
                  <div>
                    ...
                    <span className="truncate max-w-[100px] inline-block">{cardName || 'VALID NAMES'}</span>
                  </div>
              ...
              <div className="md:col-span-3 premium-glass p-sm rounded-2xl border border-white/5 space-y-xs">
                <div className="flex items-center gap-2xs border-b border-white/5 pb-2xs">
                  ...
                  <h3 className="text-xs font-bold uppercase tracking-wider text-white">Stripe Vault Interface</h3>
                </div>
          ...
          {activeStep === 5 && (
            ...
            >
              ...
              <div className="space-y-3xs">
                ...
                <p className="text-xs text-vantage-muted max-w-sm mx-auto">Your premium itinerary has been captured on-ledger. E-tickets and smart check-in parameters were dispatched to your workspace.</p>
              </div>
              <div className="p-sm bg-black/20 rounded-xl border border-white/5 inline-block text-left font-mono text-xs text-white space-y-2xs">
                <div><span className="text-vantage-muted">PASSENGER:</span> {passenger?.lastName}, {passenger?.firstName}</div>
                <div><span className="text-vantage-muted">LOCATOR:</span> VX-{Math.floor(100000 + Math.random() * 900000)}</div>
                <div><span className="text-vantage-muted">ALLOCATION:</span> {selectedSeats.join(', ')}</div>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <aside className="premium-glass rounded-2xl p-sm border border-white/5 space-y-sm sticky top-24 self-start">
        <div className="flex items-center gap-2xs pb-2xs border-b border-white/5">
          <PlaneTakeoff className="w-4 h-4 text-vantage-accent" />
          <h3 className="font-semibold text-xs tracking-wide uppercase text-white">Fare Computation</h3>
        </div>
        ...
        <div className="p-2xs rounded-xl bg-white/5 border border-white/5 flex gap-2xs items-start text-[10px] text-vantage-muted">
          ...
          <p>This transaction matrix utilizes cryptographic token signing. Pricing conditions stay locked down for a 15-minute window.</p>
        </div>
      </aside>
