  ...
  return (
    <div className="mx-auto max-w-4xl space-y-lg px-sm py-xl">
      <div className="text-center space-y-sm">
        ...
        <p className="text-vantage-muted">View your itinerary, select seats, or modify your premium travel arrangements.</p>
      </div>

      {!booking ? (
        <form onSubmit={handleSearch} className="max-w-md mx-auto space-y-md premium-glass p-lg rounded-3xl border border-white/10">
          <div>
            <label className="block text-xs uppercase tracking-wider text-vantage-muted font-bold mb-2xs">Booking Reference (PNR)</label>
            <input 
              type="text" 
              value={pnr}
          ...
          <div>
            <label className="block text-xs uppercase tracking-wider text-vantage-muted font-bold mb-2xs">Last Name</label>
            <input 
              type="text" 
              value={lastName}
        ...
        <AnimatePresence>
          ...
          >
                  ...
                  <div className="bg-vantage-dark/50 p-md rounded-2xl border border-white/5">
                    ...
                    {booking.outbound && (
                      <div className="flex justify-between items-center mb-xs">
                        ...
                        <span className="font-bold text-white">{booking.outbound.departure.iata} → {booking.outbound.arrival.iata}</span>
                      </div>
                    )}
                    {booking.returnFlight && (
                      <div className="flex justify-between items-center">
                        <span className="text-vantage-muted">Return:</span>
                        <span className="font-bold text-white">{booking.returnFlight.departure.iata} → {booking.returnFlight.arrival.iata}</span>
                      </div>
                    )}
                  </div>
                ...
                <div className="bg-vantage-dark/50 p-md rounded-2xl border border-white/5">
                   <div className="flex items-center gap-2xs text-vantage-accent mb-sm">
                    ...
                    <div className="flex flex-wrap gap-sm">
                       <button onClick={() => navigate('/tracker', { state: { flightNumber: booking.outbound?.id.split('-')[3] || 'EK201' }})} className="px-md py-sm bg-vantage-accent/10 border border-vantage-accent/20 text-vantage-accent rounded-xl hover:bg-vantage-accent hover:text-vantage-dark transition-colors font-medium">
                         Track Flight
                       </button>
            ...
            <div className="text-center">
              <button onClick={() => setBooking(null)} className="text-sm text-vantage-muted hover:text-white transition-colors underline">
