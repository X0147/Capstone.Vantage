...
export default function ConfirmationPage() {
  ...
  const { bookingReference, selectedOutbound, passengers, selectedSeats, resetStore } = useBookingStore();
  ...
  if (!bookingReference) {
    return <div className="text-center mt-20">No booking found. <button onClick={handleDone} className="text-brand-accent underline">Return Home</button></div>;
  }
      ...
      <div className="bg-white text-brand-dark p-8 rounded-2xl shadow-xl print:shadow-none print:p-0">
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            ...
            <div className="font-bold text-lg">{pass.firstName} {pass.lastName}</div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Flight</div>
            <div className="font-bold text-lg">{selectedOutbound?.airline?.name} ({selectedOutbound?.id.split('-')[3]})</div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center bg-gray-100 p-6 rounded-xl mb-8">
          <div className="text-center">
            ...
            <div className="text-sm text-gray-500">{selectedOutbound?.departure?.time?.split('T')[1].substring(0,5)}</div>
          </div>
          ...
          <div className="text-center">
            ...
            <div className="text-sm text-gray-500">{selectedOutbound?.arrival?.time?.split('T')[1].substring(0,5)}</div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-end border-t pt-6 mt-6">
          <div>
            ...
            <div className="text-2xl font-bold text-brand-emerald">{selectedSeats?.outbound?.[0] || 'Unassigned'}</div>
          </div>
        </div>
      </div>

      <div className="mt-8 flex gap-4 justify-center no-print">
        <button onClick={handlePrint} className="bg-gray-700 hover:bg-gray-600 text-white font-bold py-3 px-8 rounded-lg">
          Print Receipt
        </button>
        <button onClick={handleDone} className="bg-brand-accent hover:bg-brand-accent/80 text-white font-bold py-3 px-8 rounded-lg">
          Done
        </button>
      </div>
    </div>
