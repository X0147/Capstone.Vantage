    ...
    <div className="max-w-2xl mx-auto mt-10">
      <div className="bg-white/10 backdrop-blur-md border border-white/20 p-8 rounded-2xl shadow-xl">
        ...
        <div className="mb-6 p-4 bg-brand-dark/50 rounded-lg border border-brand-accent/30 flex justify-between">
          ...
          <span className="font-bold text-xl text-brand-emerald">${selectedOutbound?.price || 0}</span>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm mb-1">{t('payment.nameOnCard')}</label>
            <input required type="text" className="w-full bg-brand-dark/50 border border-white/20 rounded p-3 text-white" 
              value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
          </div>
          <div>
            <label className="block text-sm mb-1">{t('payment.cardNumber')}</label>
            <input required type="text" maxLength="16" className="w-full bg-brand-dark/50 border border-white/20 rounded p-3 text-white" 
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required type="text" maxLength="5" className="w-full bg-brand-dark/50 border border-white/20 rounded p-3 text-white" 
            ...
            <div>
              ...
              <input required type="password" maxLength="4" className="w-full bg-brand-dark/50 border border-white/20 rounded p-3 text-white" 
          ...
          <div className="mt-8 flex justify-end">
            ...
            >
              {isProcessing ? t('payment.processing') : t('payment.pay', { amount: selectedOutbound?.price || 0 })}
            </button>
          </div>
        </form>
      </div>
