    ...
    <footer className="border-t border-white/5 pt-md pb-sm space-y-md">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-md">
        ...
        <div className="space-y-xs text-xs">
          ...
          <ul className="space-y-2xs font-mono text-[10px] text-vantage-muted">
            <li><AccessibleButton ariaLabel="New York → London" className="hover:text-white transition-colors" onClick={() => navigate('/')}>New York → London</AccessibleButton></li>
            <li><AccessibleButton ariaLabel="New York → Tokyo" className="hover:text-white transition-colors" onClick={() => navigate('/')}>New York → Tokyo</AccessibleButton></li>
            <li><AccessibleButton ariaLabel="New York → Paris" className="hover:text-white transition-colors" onClick={() => navigate('/')}>New York → Paris</AccessibleButton></li>
          </ul>
        </div>
        <div className="space-y-xs text-xs">
          <h5 className="font-bold uppercase tracking-wider text-white">Airspace Utilities</h5>
          <ul className="space-y-2xs font-mono text-[10px] text-vantage-muted">
            <li><AccessibleButton ariaLabel="ADS‑B Telemetry Radar" className="hover:text-white transition-colors" onClick={() => navigate('/tracker')}>ADS‑B Telemetry Radar</AccessibleButton></li>
            <li><AccessibleButton ariaLabel="Itinerary Retrieval" className="hover:text-white transition-colors" onClick={() => navigate('/manage-booking')}>Itinerary Retrieval</AccessibleButton></li>
            <li><AccessibleButton ariaLabel="Frequent Flyer Miles" className="hover:text-white transition-colors" onClick={() => navigate('/dashboard')}>Frequent Flyer Miles</AccessibleButton></li>
          </ul>
        </div>
        <div className="space-y-xs text-xs">
      ...
      <div className="border-t border-white/5 pt-xs flex flex-col md:flex-row justify-between items-center gap-xs font-mono text-[9px] text-vantage-muted">
        ...
        <div className="flex items-center gap-3xs">
          <svg className="h-3 w-3 text-vantage-accent" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11h-2v2H7v2h2v2h2v-2h2V9h-2V7z"/></svg>
