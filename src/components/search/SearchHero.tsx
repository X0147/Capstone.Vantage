...
export const SearchHero: React.FC = () => {
  ...
  const { origin, destination, setOrigin, setDestination, executeSearch, searchParams, setSearchParams } = useSearchStore();
  ...
  const handleSegmentChange = (idx: number, patch: Partial<MultiCitySegment>) => {
    setMultiCitySegments(
      multiCitySegments.map((s, i) => (i === idx ? { ...s, ...patch } : s))
    );
  };
        ...
        {searchParams.tripType === 'multicity' ? (
          ...
          <div className="space-y-sm">
            {multiCitySegments.map((segment, idx) => (
              <div key={idx} className="grid grid-cols-1 md:grid-cols-12 gap-xs items-end bg-white/5 p-sm rounded-2xl border border-white/5 relative">
                ...
                <div className="md:col-span-3">
                  <label className="block text-xs uppercase tracking-wider text-vantage-muted font-bold mb-2xs">Date</label>
                  <input 
                    type="date"
                    value={segment.date}
